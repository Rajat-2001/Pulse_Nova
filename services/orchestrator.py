...
Reply with ONLY the intent label. Nothing else."""},
        ...
        if "choices" not in data:
            print(f"⚠️ Unexpected LLM response: {data}")
            return "chat"
        ...
        print("⚠️ LLM timeout — falling back to chat")
        return "chat"
...
{{}}"""},
        ...
        try:
            ...
            print("⚠️ Could not parse params JSON")
            return {}

    except requests.exceptions.Timeout:
        print("⚠️ Params extraction timeout")
        return {}
    ...
    try:
        ...
        if intent == "send_email":
            ...
            if not params.get("to"):
                return "❌ I couldn't find the recipient email address. Please try again!"
            result = gmail_bot.send_email(
                to=params.get("to", ""),
                subject=params.get("subject", "No Subject"),
                body_intention=params.get("body_intention", message)
        ...
        # ✅ Modify Event
            ...
            if not params.get("event_id"):
                return "❌ I need the event ID to modify it. Please check /calendar/upcoming for the event ID!"
            result = calendar_service.modify_event(
                event_id=params.get("event_id", ""),
                summary=params.get("summary"),
                date=params.get("date"),
        ...
        # ✅ Delete Event
            ...
            params = extract_params(message, intent)
            if not params.get("event_id"):
                return "❌ I need the event ID to delete it. Please check /calendar/upcoming for the event ID!"
            calendar_service.delete_event(params.get("event_id", ""))
            return "🗑️ Event deleted successfully"
